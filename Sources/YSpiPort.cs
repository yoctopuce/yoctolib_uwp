/*********************************************************************
 *
 *  $Id: YSpiPort.cs 59641 2024-03-05 20:50:20Z mvuilleu $
 *
...
#pragma warning disable 1998
     ...
     * </para>
     */
    public async Task<int> get_shiftSampling()
    {
        int res;
        if (_cacheExpiration <= YAPIContext.GetTickCount()) {
            if (await this.load(await _yapi.GetCacheValidity()) != YAPI.SUCCESS) {
     ...
     * </para>
     */
    public async Task<int> set_shiftSampling(int  newval)
    {
        string rest_val;
        rest_val = (newval > 0 ? "1" : "0");
        await _setAttr("shiftSampling",rest_val);
        return YAPI.SUCCESS;
    }
     ...
     * </para>
     */
    public virtual async Task<List<YSpiSnoopingRecord>> snoopMessagesEx(int maxWait,int maxMsg)
    {
        string url;
        byte[] msgbin = new byte[0];
        List<string> msgarr = new List<string>();
